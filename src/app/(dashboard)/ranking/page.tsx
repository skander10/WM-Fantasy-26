        ...
        {players && players.length > 0 && (
          <div className="flex flex-col gap-2 mb-6">
            {players.slice(0, 3).map((player, i) => (
              <div
                key={player.id}
                className={`flex items-center gap-4 rounded-2xl px-4 py-3.5 border backdrop-blur-sm transition-colors ${
                  player.id === user?.id
                    ? 'bg-amber-400/15 border-amber-400/40'
                    : i === 0
                    ? 'bg-slate-900/50 border-amber-400/20'
              ...
              >
                ...
                <div className="text-right shrink-0">
                  ...
                  <p className="text-slate-400 text-xs">Punkte</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {players && players.length > 3 && (
          <div className="flex flex-col gap-2">
            {players.slice(3).map((player, i) => (
              <div
                key={player.id}
                className={`flex items-center gap-4 rounded-xl px-4 py-3 border backdrop-blur-sm ${
                  player.id === user?.id
                    ? 'bg-amber-400/15 border-amber-400/40'
                    : 'bg-slate-900/40 border-slate-700/40'
                }`}
              >
                <span className="text-slate-400 text-sm font-medium w-8 text-center">
                  {i + 4}
                </span>
